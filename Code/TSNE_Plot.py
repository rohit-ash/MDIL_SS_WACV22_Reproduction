# -*- coding: utf-8 -*-
"""
Created on Sat Sep 16 12:43:43 2023
...
def main():
        ...
        with torch.no_grad():
            for step, (images, labels, filename, filenameGt) in enumerate(dataset_loader):
                ...
                if len(torch.unique(targets)) == 17 :
                    for i in req:
                        if i not in torch.unique(targets):
                            print('out')
                            return
    ...
    sample_labels = np.arange(15)#document change from 10 to 19
    ...
    def preprocess_fn_enco(output, targets, data_name='CS', model_name='KLD', step=2):
        ...
        city_name = (filename[0].split("val/")[1]).split("/")[0]
        image_name = '017476'#(filename[0].split("val/")[1]).split("/")[1][:-25]
        filenameS = "C:/Users/Rohit/Desktop/MDIL-SS-Reproduction/tsne_plots/enco{}_{}_step{}_".format(data_name, model_name, step) + image_name + '.png'
