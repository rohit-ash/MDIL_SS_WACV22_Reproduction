...
class non_bottleneck_1d_RAP (nn.Module):
    def __init__(self, chann, dropprob, dilated, nb_tasks=1):
        ...
        self.parallel_conv_2 = nn.ModuleList(
            [nn.Conv2d(chann, chann, kernel_size=1, stride=1, padding=0, bias=True) for i in range(nb_tasks)])
...
class Decoder (nn.Module):
    ...
    def forward(self, input):
        ...
        for layer in self.layers:
            output = layer(output)
        output_penum = output  # features before the final conv layer. dim=[1, 16, 256, 512]
        # these are the feature before the softmax. dim=[1, 20, 512, 1024]
        output = self.output_conv(output)

        return output, output_penum
...
class Net(nn.Module):
    ...
    def __init__(self, num_classes=[20], nb_tasks=1, cur_task=0):
        ...
        # current_task will be nb_tasks-1 as we are training on the last task in each incremental step
    ...
    def forward(self, input, task):
        ...
        current_task = task
        output_enc = self.encoder(input)  # dim=[1, 128, 64, 128]
        output, output_penum = self.decoder[task].forward(output_enc)
        return output, output_penum, output_enc
